...
function renderViaOutputBody(state) {
  ...
  return [
    ...
    rowHtml('  Engineering & Product', via.map(v => -(v.opexByCat.engineering || 0))),
    rowHtml('  Operations & Admin',    via.map(v => -(v.opexByCat.opsAdmin || 0))),
    rowHtml('  Sales & Marketing',     via.map(v => -(v.opexByCat.salesMktg || 0))),
    rowHtml('  Infrastructure & Cloud',via.map(v => -(v.opexByCat.infra || 0))),
    rowHtml('  Legal & Compliance',    via.map(v => -(v.opexByCat.legal || 0))),
    rowHtml('  Customer Success',      via.map(v => -(v.opexByCat.customerSuccess || 0))),
    rowHtml('  Leadership',            via.map(v => -(v.opexByCat.leadership || 0))),
    rowHtml('  General & Admin',       via.map(v => -(v.opexByCat.gna || 0))),
...
function renderInvestor(data, output, notes) {
  ...
  const viaRows = [
    ...
    rowHtml('  Leadership',            via.map(v => -(v.opexByCat.leadership || 0))),
    rowHtml('  General & Admin',       via.map(v => -(v.opexByCat.gna || 0))),
    rowHtml('Total Operating Expenses', via.map(v => v.opex), { cls: 'finance-total-row' }),
    rowHtml('EBITDA', via.map(v => v.ebitda), { cls: 'finance-highlight-row' }),
    rowHtml('EBITDA margin', via.map(v => v.ebitdaMargin), { pct: true, cls: 'finance-muted-row' }),
  ].join('');
